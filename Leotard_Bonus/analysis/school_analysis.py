...
def main():
	print("*************************************************")
	print("Running NQS Calculator")
	print("*************************************************")
	input_data_path = "../data/rtn_2022.csv"
	df = pd.read_csv(input_data_path)
	df.loc[:,'Location'] = np.where(df['Host'] == df['Team'] , 'Home', 'Away')
	df = df.drop(['Opponents','Host'], axis=1)
	...
	Drop athletes who have no NQS rankings on any event
		...
		(athlete_results.at[index, 'AA'] == 0):
			# print("{} has no events with an NQS. Dropping from results table".format(row[1]))
	...
	# print("*************************************************")
	# print("Team Results (full results in output file)")
	# print(team_results)
	print("*************************************************")
	print("Individual Results (full results in output file)")
	print(athlete_results)
	print("*************************************************")
	# meets.to_csv('../data/team_results.csv')
...
def calculate_athlete(df, athlete_name, event):
	...
	if (df_athlete.shape[0] < 6):
		print("Not enough scores to calculate NQS for {} on {}".format(athlete_name, event))
		return 0
	...
	if df_athlete.loc[(df_athlete['Location'] == 'Away')].shape[0] < 3:
		print("Not enough away scores to calculate NQS for {} on {}".format(athlete_name, event))
		return 0
